...
const Register = () => {
    ...
    const submit = () => {
        ...
        if (!regex.test(userInfo.email)) {
            ...
            fetch('https://tour-travel-backend.onrender.com/register', {
                method: 'POST',
                headers: {
        ...
        <div className=' flex justify-center items-center md:h-[40rem] mb-10 md:mb-0 px-2 '>
            <div style={{ boxShadow: "0 0 20px rgba(0, 0, 0, 0.5)" }} className="grid sm:grid-cols-2 grid-cols-1  rounded-lg ">
                ...
                <div className="bg-[#faa935] w-full text-center py-4 px-10 lg:px-14">
                    ...
                    <button onClick={submit} className="bg-black text-white mt-16 w-full px-2 py-1 rounded-md font-bold hover:cursor-pointer hover:shadow-md">register {isLogin && <Navigate to={"/Home"}></Navigate>}</button>
